	...
	async function handleSubmit() {
		...
		try {
			const response = await fetch(
				API_MICROSERVICES_BASE_URL + API_MICROSERVICES_PORT + "/downloader",
				{
					method: "POST",
...
<div class="py-0.0">
	<!-- No hay necesidad de espacio adicional aquí -->
	...
	<div class="py-0.5">
		...
		{#each dateInputs as dateInput, index}
			<div class="flex items-center space-x-2 mt-2">
				<button
					class="text-white-500 hover:text-white-600 bg-red-500 hover:bg-red-600 text-white py-1 px-2.5 rounded"
					on:click={(event) => removeDateInput(index)}
				>
				...
				{#if dateInput.start && dateInput.end}
					<span
						>{new Date(dateInput.start).toLocaleDateString("es-ES")} - {new Date(
							dateInput.end
